...
public:
    ...
    // local mapping中记录当前处理的关键帧的mnId，表示当前局部BA的关键帧id。mnBALocalForKF 在map
    // point.h里面也有同名的变量。
...
protected:
    ...
    /// Grid over the image to speed up feature matching ,其实应该说是二维的,第三维的
    /// vector中保存的是这个网格内的特征点的索引
