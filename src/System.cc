  ...
  , mbDeactivateLocalizationMode(false)        //没有这个模式转换标志
{
    ...
    //运行这个局部建图线程
    mptLocalMapping = new thread(std::bind(&ORB_SLAM2::LocalMapping::Run,  //这个线程会调用的函数
    ...
    //创建回环检测线程
    mptLoopClosing = new thread(std::bind(&ORB_SLAM2::LoopClosing::Run,  //线程的主函数
                                          mpLoopCloser));                //该函数的参数
    ...
    if (bUseViewer)
    {
        ...
        //新建viewer线程
        mptViewer = new thread(std::bind(&Viewer::Run, mpViewer));
        //给运动追踪器设置其查看器
...
void System::SaveTrajectoryTUM(const string& filename)
{
    ...
    for (list<cv::Mat>::iterator lit  = mpTracker->mlRelativeFramePoses.begin(),
         ...
         lit != lend; lit++, lRit++, lT++, lbL++)  // TODO 为什么是在这里更新参考关键帧？
    {
        //如果该帧追踪失败，不管它，进行下一个
        if (*lbL)
            continue;
        ...
        // If the reference keyframe was culled（剔除）, traverse（扫描？） the spanning tree to get a suitable
        // keyframe.
        while (pKF->isBad())
        {
            //更新关键帧变换矩阵的初始值，
