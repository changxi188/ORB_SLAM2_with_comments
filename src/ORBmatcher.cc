    ...
    for (size_t iMP = 0; iMP < vpMapPoints.size(); iMP++)
    {
        ...
        for (vector<size_t>::const_iterator vit = vIndices.begin(), vend = vIndices.end(); vit != vend; vit++)
        {
            ...
            if (F.mvuRight[idx] > 0)
            {
                ...
                //相机运动时对其产生的影响也就越大, 因此需要扩大其搜索空间. 当给定缩放倍率为1.2的时候, mvScaleFactors
                //中的数据是: 1 1.2 1.2^2 1.2^3 ...
                                       ...
                                       const KeyFrame* pKF2)
{
    ...
    // 3.84 是自由度为1时，服从高斯分布的一个平方项（也就是这里的误差）小于一个像素，这件事发生概率超过95%时的概率
    // （卡方分布）
...
int ORBmatcher::SearchByProjection(Frame& CurrentFrame, const Frame& LastFrame, const float th, const bool bMono)
{
    ...
    // 判断前进还是后退
    const bool bForward =
        tlc.at<float>(2) > CurrentFrame.mb && !bMono;  // 非单目情况，如果Z大于基线，则表示相机明显前进
    const bool bBackward =
        -tlc.at<float>(2) > CurrentFrame.mb && !bMono;  // 非单目情况，如果-Z小于基线，则表示相机明显后退
