 ...
 * Motion and structure from motion in a piecewise planar environment. International Journal of Pattern Recognition and
                               ...
                               int minTriangulated)
{
    ...
    // Instead of applying the visibility constraints proposed in the WFaugeras' paper (which could fail for points seen
 ...
 */
void Initializer::Triangulate(const cv::KeyPoint& kp1,  //特征点, in reference frame
 ...
 */
void Initializer::Normalize(const vector<cv::KeyPoint>& vKeys, vector<cv::Point2f>& vNormalizedPoints,
                            cv::Mat& T)  //将特征点归一化的矩阵
{
                         ...
                         vector<bool>& vbGood, float& parallax)
{
    ...
    for (size_t i = 0, iend = vMatches12.size(); i < iend; i++)
    {
        ...
        // depth) 如果深度值为负值，为非法三维点跳过该匹配点对 ?视差比较小时，重投影误差比较大。这里0.99998
        // 对应的角度为0.36°,这里不应该是 cosParallax>0.99998 吗？ ?因为后面判断vbGood 点时的条件也是
        // cosParallax<0.99998 !可能导致初始化不稳定
        if (p3dC1.at<float>(2) <= 0 && cosParallax < 0.99998)
            continue;

        // Check depth in front of second camera (only if enough parallax, as "infinite" points can easily go to
        // negative depth) 讲空间点p3dC1变换到第2个相机坐标系下变为p3dC2
