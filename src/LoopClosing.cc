...
bool LoopClosing::DetectLoop()
{
    ...
    // 为3等;具体反映在数据类型 ConsistentGroup.second上 连续组(Consistent group):
    // mvConsistentGroups存储了上次执行回环检测时,
...
void LoopClosing::CorrectLoop()
{
    ...
    // 对地图点操作
    {
        ...
        // the loop Step 2.2：得到矫正的当前关键帧的共视关键帧位姿后，修正这些共视关键帧的地图点
        ...
        for (KeyFrameAndPose::iterator mit = CorrectedSim3.begin(), mend = CorrectedSim3.end(); mit != mend; mit++)
        {
            ...
            // 其实是现在已经有了更新后的关键帧组中关键帧的位姿,但是在上面的操作时只是暂时存储到了 KeyFrameAndPose
            // 类型的变量中,还没有写回到关键帧对象中 调用toRotationMatrix 可以自动归一化旋转矩阵
